...
model.add(Convolution2D(nb_filters2, conv2_size, strides = conv2_size, padding ="same"))
model.add(Activation('relu'))
...
model.add(Convolution2D(nb_filters3, conv3_size, strides=conv3_size, padding ='same'))
model.add(Activation('relu'))
#model.add(MaxPooling2D(pool_size=(pool_size, pool_size), dim_ordering='th'))
model.add(MaxPooling2D(pool_size=(pool_size, pool_size), data_format='channels_last'))
...
target_dir = "../models/weights-improvement-{epoch:02d}-{acc:.2f}.hdf5"
...
checkpoint = ModelCheckpoint(target_dir, monitor='acc', verbose=1, save_best_only=True, mode='max')
